...
class RNN(nn.Module):
    def __init__(self, vocab_size, emb_dim, hidden_size, output_size, word_id_dict, device=None):
        super().__init__()
        ...
        for word, idx in word_id_dict.items():
            if word in model:
                weight[idx] = torch.tensor(model[word], device=device)
...
def train_fn(
    ...
    for dataloader_x, dataloader_y in loader:
        ...
        dataloader_y_pred_prob = model(
            x=dataloader_x,
            h_0=torch.zeros(2 * 3, BATCHSIZE, HIDDEN_SIZE,
...
# データの読み込み
train = pd.read_csv("./train_title_id.csv")
test = pd.read_csv("./test_title_id.csv")
...
# 辞書の読み込み
with open(f"./word_id_dict.pkl", "rb") as tf:
...
# train_acc: 0.7920449227889564
# test_acc: 0.7908545727136432
# 3m43s

# train_acc: 0.7919513336452971
# test_acc: 0.7908545727136432
# 3m34s
